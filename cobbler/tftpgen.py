...
class TFTPGen(object):
    ...
    def copy_bootloaders(self, dest):
        ...
        utils.subprocess_call(self.logger, ["rsync", "-rpt", "--copy-links", "--progress",
                                            "--exclude=.cobbler_postun_cleanup", "{src}/".format(src=src), dest], shell=False)
        src = self.settings.grubconfig_dir
        utils.subprocess_call(self.logger, ["rsync", "-rpt", "--copy-links", "--progress",
                                            "--exclude=README.grubconfig", "{src}/".format(src=src), dest], shell=False)
