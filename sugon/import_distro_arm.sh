...
ImportDistroX86_64 "${MIX_NAME}" "${MIX_NAME}" "vmlinuz" "initrd.img"
ImportProfile "${MIX_NAME}" "${MIX_NAME}" "console=tty boot=live union=overlay livecd-installer quiet splash -- root=/dev/nfs nfsroot=172.16.0.2:/public/ft/uos_sp1_desktop"
#
MIX_NAME="Phytium_UOS_V20_SP1_Desktop"
ImportDistroX86_64 "${MIX_NAME}" "${MIX_NAME}" "vmlinuz" "initrd.img"
ImportProfile "${MIX_NAME}" "${MIX_NAME}" "console=tty boot=live union=overlay livecd-installer quiet splash -- root=/dev/nfs nfsroot=172.16.0.2:/public/ft/uos_sp1_desktop"
